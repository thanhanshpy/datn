...
namespace Dialouge.LogicalLines
{
    public static class LogicalLineUtilities 
    {
        ...
        public static class Expressions
        {
            ...
            public static readonly string regexArithmatic = @"([-+*/=]=?)";
            public static readonly string regexOperatorLine = @"^\$\w+\s*(=|\+=|-=|\*=|/=|)\s*";

            public static object CaculateValue(string[] expressionParts)
            {
                ...
                CaculateValue_DivisionAndMultiplication(operatorStrings, operands);

                CaculateValue_AdditionAndSubtraction(operatorStrings, operands);

                return operands[0];
            }

            private static void CaculateValue_DivisionAndMultiplication(List<string> operatorStrings, List<object> operands)
            {
                    ...
                    if(operatorString == "*" || operatorString == "/")
                    {
                        ...
                        else
                        {
                            if(rightOperand == 0)
                            {
                                if(rightOperand == 0)
                                {
                                    Debug.LogError("cant divine by zero");
                                }
                                operands[i] = leftOperand / rightOperand;
                            }
                        }
            ...
            private static void CaculateValue_AdditionAndSubtraction(List<string> operatorStrings, List<object> operands)
            {
                for(int i = 0; i < operatorStrings.Count;i++)
                {
                    string operatorString = operatorStrings[i];
        ...
        public static class Conditions
        {
            ...
            public static bool EvaluateCondition(string condition)
            {
                ...
                for (int i = 0; i < parts.Length; i++)
                {
                    ...
                    {
                        parts[i] = parts[i].Substring(1, parts[1].Length - 2);
                    }                  
                }
