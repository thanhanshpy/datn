...
public class TagManager 
{
    ...
    private static string InjectTags(string value)
    {
        if (tagRegex.IsMatch(value))
        {
            ...
            {
                if (tags.TryGetValue(match.Value, out var targetValueRequest))
                {
                    value = value.Replace(match.Value, targetValueRequest());
                }
            }
        }
    ...
    private static string InjectVariables(string value)
    {
        ...
        for(int i = matchesList.Count - 1; i >= 0; i--)
        {
            ...
            bool endsInIllegalCharacter = variableName.EndsWith(VariableStore.databaseVariableRelationalID);
